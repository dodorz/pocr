...
- **模型目录**：发布安装默认读取 `pocr.exe` 同级的 `models\`；源码开发时兼容 `build\..\models\` 和当前工作目录的 `models\`。也可用 `--models-dir DIR` 指定任意位置。模型文件需先执行 `scripts\fetch_models.bat` 下载，或将 `models\` 目录复制到安装目录。
